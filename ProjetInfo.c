...
void viderBuffer() {                   // procédure pour vider le buffer des entrées clavier
...
void afficherTexte(char *chaine, int nombreCaractereTotal) {  /*procédure pour afficher une chaine de caractère en occupant un nombre
	donné de caractère total en console */
...
void afficherEntier(int entier, int nombreCaractereTotal) {       //procédure pour afficher un entier en occuant un nombre donné de caractère en console
...
void insertionCommande(ListeCommande *listeCommande, Commande *commande) { //Procédure pour insérer les nouvelles commandes
...
void afficherinformationCocktail(Cocktail *cocktail, char *texte) { /*procédure pour afficher les information d'un cocktail
	                                                                avec un texte particulier en début de tableau en console*/
...
void afficherInformationBoisson(ListeBoisson *listeBoisson, char *texte) {   /*procédure pour afficher les information des boissons d'une liste
	                                                                         avec un texte particulier en début de tableau en console*/
...
void afficherListeBoisson(ListeBoisson *listeBoisson, ListeCocktail *listeCocktail) {  //Procédure pour afficher les information des boissons et des cocktails
    ...
    if (listeCocktail != NULL) {             //même chose que pour afficherInformation boisson sauf que l'on prend un cocktail comme variable
        ...
        while (cocktailactuel != NULL) {
            ...
            if (cocktailEnStock(cocktailactuel, listeBoisson)) {     /*Si le stock de boisson est nul, alors un cocktail
				                                                     qui contient cette boisson n'est plus disponible non plus*/
                printf("*    RUPTURE DE STOCK\n");                   //On affiche donc "rupture de stock"
...
void afficherListeCommandes(ListeCommande *listeCommande) {   //Procédure pour afficher l'historique des commandes
    ...
    printf("Les clients ont commandé: \n");
...
Boisson *obtenirBoisson(ListeBoisson *liste, char *nom) {     //fonction pour obtenir une boisson dans une liste

    Boisson *actuel = liste->premier;                         //on récupère la première boisson de la liste donnée

    while (actuel != NULL) {                                  //on vérifie que la boisson existe

        if (strcmp(actuel->nom, nom) == 0) {                 //si la boisson a le nom recherché
...
Cocktail *obtenirCocktail(ListeCocktail *listeCocktail, char *nom) {// fonction pour obtenir un cocktail dans une liste
...
void gererStocks(ListeBoisson *listeBoisson) {                 //Procédure pour gérer les stocks

    afficherInformationBoisson(listeBoisson, "Liste Boisson"); //On affiche la liste des boissons et leurs stocks
    afficherRetourLigne(3);
...
void creationCocktail(ListeBoisson *listeBoisson, ListeCocktail *listeCocktail, int client) {  //Procédure pour créer un nouveau cocktail

    char *nom = malloc(50 * sizeof(char));                  //on alloue la place en mémoire pour notre cocktail
    int condition = 1;

    while (condition == 1) {                                //à répéter tant que le nom n'est pas bon
    ...
    cocktail.listeBoisson = initialisation();              //on initialise la liste de boissons du cocktail
    ...
    while (condition == 0) {                               //on répète tant que le créateur veut ajouter un boisson
        ...
        if (listeBoissonContient(listeBoisson, nomBoisson) == 0 || (client == 1 && obtenirBoisson(listeBoisson, nomBoisson)->nombreenstock <= 0)) {
		/*si la boisson voule n'est pas contenue dans la liste de boissons proposées
		ou que le créateur est un client et que la boisson est en rupture de stock*/
            printf("Nous ne proposons pas cette boisson\n");    //Alors on affiche ce message
        } else {

            Boisson *boisson = obtenirBoisson(listeBoisson, nomBoisson);      //sinon, on récupère la boisson

                //et on met à jour les inforamtion du cocktail
                alcool += boisson->alcool;
                sucre += boisson->sucre;
                prix += boisson->prix;
                compteur++;

                //puis on ajoute la nouvelle boisson à la lsite de boisson du cocktail
    ...
    //on initialise les informations du cocktail avec celles calculées dans la procédure
    cocktail.nom = nom;
    ...
    //on insère le nouveau cocktail dans la liste des cocktails
    insertionCocktail(listeCocktail, cocktail.listeBoisson, nom, alcool / compteur, sucre / compteur,(prix + (prix / 10)));

    afficherRetourLigne(3);
}

void ajouterBoisson(ListeBoisson *listeBoisson) {              //Ajout d'une nouvelle boisson

    char *nomBoisson = malloc(50 * sizeof(char));             //on créé le nom de la boisson

    printf("Entrez le nom de votre boisson (30 caracteres max)\n");
        lireEntreeConsole(nomBoisson, 50);

    if (strlen(nomBoisson) > 30) {                           // si le nom entré n'est pas valable
    ...
    //on insère la nouvelle boisson dans la liste des boissons proposées par le bar
...
void afficherMenuBarman(ListeBoisson *listeBoisson, ListeCocktail *listeCocktail, ListeCommande *listeCommande, //Affichage du menu barman
                        int *chiffreAffaire) {
    ...
    while (1 == 1) {
        ...
        switch (choice) {
            ...
            case 1: /*Si choix 1*/
                choice = 0;
                afficherListeCommandes(listeCommande);            //Appel de afficherListeCommandes pour afficher la liste de commandes
                break;

            case 2: /*Si choix 2*/
                choice = 0;
                creationCocktail(listeBoisson, listeCocktail, 0); //Appel de creationCocktail qui créée les cocktails
                break;

            case 3: /*Si choix 3*/
                choice = 0;
                gererStocks(listeBoisson);                       //Appel de gererStocks pour gérer les stocks
                break;
            ...
            case 6: /*Si choix 6*/
                ...
                choice = 0;
                ajouterBoisson(listeBoisson);                     //Appel de la ajouterBoisson qui ajoute une nouvelle boisson
                break;
...
Commande *commanderBoisson(ListeBoisson *listeBoisson, ListeCocktail *listeCocktail, int *chiffreAffaire) { //Commandes par les clients
    ...
    if (listeBoissonContient(listeBoisson, nomBoisson) == 1) {
        ...
        if (obtenirBoisson(listeBoisson, nomBoisson)->nombreenstock > 0) {   //Si la BOISSON commandée est en stock

            Boisson *boisson = obtenirBoisson(listeBoisson, nomBoisson);
            boisson->nombreenstock -= 1;                                     //Le stock de la boisson diminue de 1

            Commande *commande = malloc(sizeof(*commande));                  //La commande est prise en compte
            commande->nom = boisson->nom;
            commande->suivant = NULL;
            *chiffreAffaire += boisson->prix;                                //Le chiffre d'affaire est mis à jour
            printf("Vous avez commandé 1 %s, vous serez servi rapidement\n", boisson->nom);
                        printf("Le prix de votre commande est de %i euros\n", boisson->prix);
                        return commande;

        } else {                                                            // Sinon on affiche que la boisson n'est plus en stock

            printf("Nous n'avons plus de %s en stock\n", nomBoisson);
                    }

    } else if (listeCocktailContient(listeCocktail, nomBoisson)) {         //On refait pareil pour les cocktails
...
void afficherMenuClient(ListeBoisson *listeBoisson, ListeCocktail *pCocktail, ListeCommande *listeCommandes, //Affichage du menu client
                        int *chiffreAffaire) {
    ...
    while (condition == 1) {
        ...
        printf("Vous êtes dans le menu client\n");
                ...
                printf("2. Voir la carte des boissons\n");
...
void afficherMenu(ListeBoisson *listeBoisson, ListeCocktail *listeCocktail, ListeCommande *listeCommandes, //Affichage du menu principal
                  int *chiffreAffaire) {
...
int main() {                                                 // Fonction principale
    ...
    insertion(listeBoisson, "tequila", 35, 0, 8, 100);      //Insertion des boissons pré-créées avec nom, degré d'alcool, degré de sucre, prix, stock
    insertion(listeBoisson, "vodka", 37, 0, 9, 100);
    insertion(listeBoisson, "whisky", 40, 0, 10, 100);
    insertion(listeBoisson, "rhum", 37, 0, 4, 100);
    insertion(listeBoisson, "biere", 12, 3, 3, 100);
    insertion(listeBoisson, "vin", 12, 13, 6, 50);
    insertion(listeBoisson, "fanta", 0, 30, 3, 150);
    insertion(listeBoisson, "sprite", 0, 30, 3, 150);
    insertion(listeBoisson, "coca", 0, 46, 2, 300);
    insertion(listeBoisson, "noix de coco", 0, 10, 8, 80);
    insertion(listeBoisson, "jus multifruits", 0, 45, 4, 200);
    insertion(listeBoisson, "jus d'orange", 0, 10, 3, 200);
    insertion(listeBoisson, "jus de pomme", 0, 27, 3, 200);
    insertion(listeBoisson, "grenadine", 0, 60, 3, 200);
    insertion(listeBoisson, "limonade", 0, 14, 2, 200);
    insertion(listeBoisson, "eau petillante", 0, 0, 1, 125);
    insertion(listeBoisson, "eau plate", 0, 0, 1, 400);
